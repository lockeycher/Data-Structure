...
public class MyLinked {
    ...
    private static double max (Node x) {
    	...
    	if (x == null) {
    		...
    		return maxNumber; //at end and in the middle with wrong number
    	}
    	...
    	if (x == null) {
    		...
    		if (x.next != null) {
    			if (x.item < next.item) {
    				result = next.item;
    				x = x.next;
    				maxRecursive(x, result);
    			}	
    		}	
    	}
        return result; //at and and in the middle with wrong number
    ...
    public void remove (double item) {
        ...
        if (N == 1 && first.item == item) {
            ...
            Node oldNode = first;
            Node currentNode = oldNode.next;       // start from second node
            int counter = 0;
            for (int i = 0; i < N-1; i++){
                if (currentNode.item == item){
                    oldNode.next = currentNode.next;     // skip next node and advance
                    counter++;
                } else {        // if item not match
                    // advance node normally
                    oldNode = currentNode;
                }
